...
class SubjectBase(models.Model):
    ...
    entrustment_company = fields.Char('委託單位')
    ...
    check_number = fields.Char('送驗單編號')
    execute_date = fields.Date('執行時間')
    patient_number = fields.Char('病歷編號')
    sample_number = fields.Char('樣品編號')
    lab_manager = fields.Char('實驗室主管')
    report_sign = fields.Char('報告簽署人')

    #定序實驗結果說明
    sample_id = fields.Char('樣本編號')
    ...
    Prelim_lib = fields.Char('初步集庫讀數')
    Comparison_data = fields.Char('比對資料庫')
    human_reading = fields.Char('比對至人類序列讀數')
    human_percent = fields.Char('比對至人類序列比例')
    target_reading = fields.Char('比對至目標序列讀數')
    target_percent = fields.Char('比對至目標序列比例')
    sequence_cover = fields.Char('10%分位序列覆蓋率')
...
class GeneReportDocx(GeneReportWord):
    ...
    def generate_word_report(self, document, data, objs, report):
        ...
        for line in objs:
                ...
                gene_data_fir.append({'gene_fir': line.gene_impact_code[0].gene,'codingConsequence_fir':line.gene_impact_code[0].codingConsequence,
                                       'var_percent_fir':line.gene_impact_code[0].var_percent, 'protein_fir':line.gene_impact_code[0].new_protein})
                print gene_data_fir
                ...
                document.merge(
                    ...
                    Prelim_lib=line.Prelim_lib,
                    Comparison_data=line.Comparison_data,
                    human_reading=line.human_reading,
                    human_percent=line.human_percent,
                    target_reading=line.target_reading,
                    target_percent=line.target_percent,
                    sequence_cover=line.sequence_cover,
                ...
                document.merge_rows('gene_fir', gene_data_fir)
